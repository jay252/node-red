RED.tourGuide = (function() {
    ...
    function runTourStep(step, state, done) {
        ...
        prepareStep(step, state, function() {
            ...
            if (step.titleImage) {
                $('<div style="margin: auto;width: 50%;margin-bottom:20px;border-radius:6px;display: flex;background-color: rgb(67, 150, 190);width: 64px;height: 64px;align-items: center;justify-content: center;"><img style="width: 40px;height: 40px;" src="red/images/s.svg"></div>').appendTo(stepDescription);
            }
            if (step.title) {
                $('<h2>').text(getLocaleText(step.title)).appendTo(stepDescription);
            }
            $('<div>').css("text-align","center").html(getLocaleText(step.description)).appendTo(stepDescription);
