module.exports = function(RED) {
    ...
    function DebugNode(n) {
        ...
        this.on("input", function(msg, send, done) {
            ...
            } else if (n.targetType === "flow"){
                if (!this.active || !this.tosidebar) {
                    done();
                    return;
                }
                var contextKey = RED.util.parseContextStore(n.complete);
                var target = node.context()['flow'];
                target.get(contextKey.key, contextKey.store, (err,fromValue) => {
                    if (err) {
                        done(err)
                    } else {
                        //getFromValueType(fromValue,done);
                        sendDebug({id:node.id, z:node.z, _alias: node._alias,  path:node._flow.path, name:node.name, topic:msg.topic, msg:fromValue});
                        //sendDebug({msg:fromValue});
                    }
                });
            } else {
                prepareValue(msg,function(err,debugMsg) {
